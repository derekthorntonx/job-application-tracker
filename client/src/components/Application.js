...
function Application( {application, setApplications} ) {
    ...
    async function fetchUpdatedList() {
        const response = await fetch(process.env.REACT_APP_FETCH_URL + `/applications/${user.email}`, {method: 'GET',
        headers: {'Authorization': user.token}})
    ...
    return (
        ...
        : <li className='expandedViewAllApplication'>
            ...
            <div style={{gridColumn: '1', marginBottom: '1rem'}}><a href={`${application.postingLink}`} target="_blank" rel='noreferrer noopener'>{application.postingLink}</a></div>
